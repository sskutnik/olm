...
class TestThinningFunction:
    ...
    def test_generate_thinned_burnup_list_always_keep_ends(self):
        ...
        result = assemble._generate_thinned_burnup_list(
            5, burnup_list, always_keep_ends=True
        )
    ...
    @pytest.mark.parametrize(
        "burnup_list,keep_every,expected",
        [
    ...
    def test_generate_thinned_burnup_list_parametrized(
        self, burnup_list, keep_every, expected
    ):
...
class TestArpdataTxtFunction:
    ...
    def test_arpdata_txt_fuel_types(self):
        ...
        with patch("scale.olm.assemble._get_arpinfo"), patch(
            "scale.olm.assemble._generate_thinned_burnup_list"
        ), patch("scale.olm.assemble._process_libraries"), patch("pathlib.Path"):
            # Mock returns to avoid actual file operations
            with patch("scale.olm.assemble._get_arpinfo") as mock_arpinfo, patch(
                "scale.olm.assemble._generate_thinned_burnup_list"
            ) as mock_thin, patch(
                "scale.olm.assemble._process_libraries"
            ) as mock_process:
                mock_arpinfo.return_value = Mock(
                    burnup_list=[0, 10, 20], get_space=Mock(return_value={})
                )
...
class TestArchiveFunction:
    ...
    def test_archive_function_structure(self):
        ...
        with patch("builtins.open", mock_open_json()), patch(
            "subprocess.run"
        ) as mock_subprocess, patch("pathlib.Path") as mock_path_class:
...
def mock_open_json():
    ...
    json_data = {
        "perms": [
            {"input_file": "case1.inp", "state": {"enrichment": 3.0, "mod_dens": 0.7}},
            {"input_file": "case2.inp", "state": {"enrichment": 4.0, "mod_dens": 0.8}},
        ]
    }
