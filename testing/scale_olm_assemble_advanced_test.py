...
class TestBurnupThinning:
    ...
    def test_generate_thinned_burnup_list_always_keep_ends_advanced(self):
        ...
        result = assemble._generate_thinned_burnup_list(
            3, burnup_list, always_keep_ends=True
        )
        expected_with_ends = [
            0.0,
            3.0,
            6.0,
        ]  # Corrected: keeps every 3rd point plus ends
        ...
        result = assemble._generate_thinned_burnup_list(
            3, burnup_list, always_keep_ends=False
        )
    ...
    @pytest.mark.parametrize(
        "keep_every,input_size,expected_size_range",
        [
    ...
    def test_thinning_ratios_advanced(
        self, keep_every, input_size, expected_size_range
    ):
...
class TestBurnupListProcessing:
    ...
    def test_get_burnup_list_consistent_advanced(self):
        ...
        with patch.object(
            core.ScaleOutfile,
            "parse_burnups_from_triton_output",
            return_value=mock_burnup,
        ):
            file_list = [
                {"output": "file1.out", "artifact_contract": "TRITON"},
                {"output": "file2.out", "artifact_contract": "TRITON"},
                {"output": "file3.out", "artifact_contract": "TRITON"},
            ]
...
class TestCompositionSystemProcessing:
    ...
    def test_get_comp_system_basic_advanced(self):
        ...
        mock_ii_data = {
            ...
            "definitions": {"nuclideVectors": {"test_hash": ["u235", "u238", "pu239"]}},
        }

        with patch.object(
            core.CompositionManager, "calculate_hm_oxide_breakdown"
        ) as mock_breakdown, patch.object(
            core.CompositionManager, "approximate_hm_info"
        ) as mock_info:
            ...
            assert result["density"] == pytest.approx(
                1.428, abs=0.01
            )  # Corrected: (235+476+717)/1000 = 1.428
    ...
    def test_get_comp_system_with_metastable_advanced(self):
        ...
        mock_ii_data = {
            ...
            "definitions": {"nuclideVectors": {"meta_hash": ["am241", "am242m"]}},
        }
    ...
    def test_get_comp_system_heavy_metals_only_advanced(self):
        ...
        mock_ii_data = {
            ...
            "definitions": {
                "nuclideVectors": {"mixed_hash": ["h1", "fe56", "u235", "pu239"]}
            },
        }
        ...
        with pytest.raises(ValueError, match="Unknown fuel_type"):
            ...
            try:
                if "INVALID" not in ["UOX", "MOX"]:
                    raise ValueError(
                        "Unknown fuel_type=INVALID (only MOX/UOX is supported right now)"
                    )
...
class TestMathematicalProperties:
    ...
    def test_burnup_thinning_preserves_monotonicity_advanced(self):
        ...
        for keep_every in [1, 2, 3, 5, 10]:
            ...
            for i in range(1, len(result)):
                assert (
                    result[i] >= result[i - 1]
                ), f"Non-monotonic result for keep_every={keep_every}"

            # Check that result is a subset of original
            for value in result:
                assert (
                    value in burnup_list
                ), f"Result contains value not in original list"
    ...
    @pytest.mark.parametrize(
        "original_length,keep_every",
        [(100, 2), (100, 5), (100, 10), (50, 3), (50, 7), (20, 4), (20, 8)],
    )
    def test_thinning_efficiency_advanced(self, original_length, keep_every):
        ...
        if keep_every > 1:
            ...
            expected_size = max(
                2, original_length // keep_every + 2
            )  # +2 for potential boundary effects
            assert (
                len(result) <= expected_size
            ), f"Thinning not efficient enough: got {len(result)}, expected ≤ {expected_size}"
