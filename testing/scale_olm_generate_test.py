...
def test_constpower_burndata_sorts_burnups_and_handles_single_point():
    ...
    assert olm.generate.time.constpower_burndata({"specific_power": 40.0}, [0.0]) == {
...
def test_jt_expander_records_rendered_input_classification(tmp_path):
    config_file = tmp_path / "config.olm.json"
    config_file.write_text("{}")
    template_file = tmp_path / "model.jt.inp"
    template_file.write_text("=t6-depl-shift parm=(bonami)\nend\n")
    ...
    result = olm.generate.root.jt_expander(
        ...
        states={"_type": "scale.olm.generate.states:full_hypercube", "x": [1.0]},
        comp={},
        time={"_type": "scale.olm.generate.static:pass_through"},
        artifact_contract="TRITON",
        _model={"name": "classification-test"},
        _env={"config_file": str(config_file), "work_dir": str(work_dir)},
    )

    perm = result["perms"][0]
    assert perm["_scale"]["artifact_contract"] == "TRITON"
    assert perm["_scale"]["sequences"] == ["t6-depl-shift"]

    with open(work_dir / perm["_"]["data_file"], "r") as f:
        data_file = json.load(f)
    assert data_file["_scale"] == perm["_scale"]
...
def test_jt_expander_rejects_artifact_contract_mismatch(tmp_path):
    ...
    config_file.write_text("{}")
    template_file = tmp_path / "model.jt.inp"
    template_file.write_text("=polaris\nend\n")

    with pytest.raises(ValueError, match="Configured artifact_contract=TRITON"):
        olm.generate.root.jt_expander(
            ...
            artifact_contract="TRITON",
            _model={"name": "classification-test"},
            _env={"config_file": str(config_file), "work_dir": str(tmp_path / "_work")},
        )
